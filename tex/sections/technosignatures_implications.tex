...
Eq. (5) gives the detectability duration in terms of the mean duty cycle and time in the observation window:
\begin{equation}
    L_{\text{eff}}=D_cT_{\text{span}}.
\end{equation}
Some technosignatures may have a lifetime exactly equal to the the civilization's lifetime, but others may persist for some time after collapse. For technosignature $i$, the detectability duration is given as
\begin{equation}
