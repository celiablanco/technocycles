...
\midrule
High & $\leq 1000$ & 1.0--2.5 & S1, S2, S4, \textcolor{red}{S7,} S8, S9 \\
...
We implement Monte Carlo simulations to explore how uncertainty in model parameters shapes the long-term behavior of each scenario. Each scenario's baseline values (Table~\ref{tab:params}) are perturbed using probabilistic variation to capture plausible uncertainty (Table~\ref{tab:uncertainty}). Initial resource stock (\(R_{0}\)) and depletion rate (\(\delta\)) are sampled from normal distributions with modest standard deviations to represent uncertainty in ecological baselines and consumption patterns. Collapse depth (\(c_{f}\)), recovery delay (\(r_{d}\)), and recovery fraction (\(r_{f}\)) are sampled using triangular distributions centered on scenario-specific values, allowing outcomes to concentrate near expected levels while still capturing asymmetries in resilience, institutional memory, and recovery capacity.
...
S7 & 0.0060 & \textcolor{red}{400} & \textcolor{red}{1.5} & 0.3 & 60 & 0.20 & 0.001 \\
S8 & \textcolor{red}{0.0015} & \textcolor{red}{800} & \textcolor{red}{1.3} & \textcolor{red}{0.6} & 70 & \textcolor{red}{0.35} & 0.0 \\
